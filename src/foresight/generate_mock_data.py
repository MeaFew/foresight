...
from foresight.config import RANDOM_STATE, RAW_DATA_DIR
...
def main():
    ...
    if len(existing) == len(real_files) and not args.force:
        logger.info(f"Real data already exists at {RAW_DATA_DIR}")
        logger.info("Skipping mock data generation. Use --force to overwrite.")
        return
    ...
    logger.info(f"Generating data for {args.n_stores} stores, {args.n_items} items ...")
    logger.info(f"Train period: {dates.min().date()} ~ {dates.max().date()}")
    logger.info(f"Test period:  {test_dates.min().date()} ~ {test_dates.max().date()}")
    ...
    logger.info(f"Stores: {len(stores)} | Items: {len(items)}")

    # Oil
    oil = generate_oil_prices(all_dates)
    oil.to_csv(RAW_DATA_DIR / "oil.csv", index=False)
    logger.info(f"Oil prices: {len(oil)} days")

    # Holidays
    holidays = generate_holidays(all_dates)
    holidays.to_csv(RAW_DATA_DIR / "holidays_events.csv", index=False)
    logger.info(f"Holidays: {len(holidays)} events")

    # Sales
    logger.info("Generating sales data (this may take a moment) ...")
    ...
    logger.info(f"Train sales: {len(train_sales):,} rows")
    ...
    logger.info(f"Test sales: {len(test_sales):,} rows")

    # Note: transactions.csv is not generated by mock data.
    # It is optional — preprocess.py skips the transactions merge
    # when the file is absent (see preprocess.py line 42 and line 111).
    # Real Kaggle data includes transactions.csv; mock data omits it.
